...
RUN apt update --fix-missing && \
    ...
    sudo sed -i 's/^elasticsearch hard memlock.*/elasticsearch hard memlock 200000/' /etc/security/limits.conf && \
    sudo sed -i 's/^ES_HEAP_SIZE=.*/ES_HEAP_SIZE=256m/' /etc/default/elasticsearch && \
    sudo sed -i 's/^MAX_LOCKED_MEMORY=.*/MAX_LOCKED_MEMORY=200000/' /etc/default/elasticsearch && \
    sudo sed -i 's/^#ES_JAVA_OPTS=.*/ES_JAVA_OPTS=-server -Xms512m -Xmx512m/' /etc/default/elasticsearch && \
    sudo sed -i 's/^index.number_of_shards:.*/index.number_of_shards: 5/; s/^index.number_of_replicas:.*/index.number_of_replicas: 1/' /etc/elasticsearch/elasticsearch.yml && \
